...
private:
    ...
    void cmd_publisher(){
        ...
        if (t_ < total_time2){
            ...
            if(cmd_interface_ == "velocity" && point_2a ==1){
                ...
                if(rot_axis=='x'){
                    ...
                    Eigen::Vector3d::UnitX()).toRotationMatrix(); //rotazione di un certo angolo intorno all'asse x
                }
                else if(rot_axis=='y'){
                    R_offset = Eigen::AngleAxisd(offset_angle, Eigen::Vector3d::UnitY()).toRotationMatrix(); //rotazione di un certo angolo intorno all'asse x
                }
                else if(rot_axis=='z'){
                    R_offset = Eigen::AngleAxisd(offset_angle, Eigen::Vector3d::UnitZ()).toRotationMatrix(); //rotazione di un certo angolo intorno all'asse x
                }
                else if(rot_axis=='n'){
        ...
        if(cmd_interface_ == "velocity" && point_2a == 2){
            ...
            L.block(0,3,3,3) = skew(s);                                                                                      //aruco_pos_n è il vettore GIA' NORMALIZZATO "s" (vedi più su)
            Eigen::Matrix3d Rc = toEigen(robot_->getEEFrame().M);
            Eigen::MatrixXd R = Eigen::Matrix<double,6,6>::Zero();
            R.block(0,0,3,3) = Rc;
            R.block(3,3,3,3) = Rc;
            L=L*R;                                                                                                         //se la formula mi dice "R", perché devo moltiplicare per "R.tranpose()"? Perché c'è scritto "mappa velocità lineari/angolari della CAMERA", quindi è nel reference frame della camera. Ma noi vogliamo comandare velocità nel base/spatial/inertial frame. Quindi la formula è sbagliata: è "R^T"!
